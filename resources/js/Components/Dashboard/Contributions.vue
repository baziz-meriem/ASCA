<template>
    <div class="p-4 flex-auto px-9 py-12">
        ...
        <div class="h-72 mb-32">
            ...
            >
                ...
                >
                    ...
                    <h2 class="capitalize text-sm">{{ item.nature}}</h2>
                    <h2 class="capitalize text-sm">{{ item.statut }}</h2>
                    <div class="flex gap-2">
                        ...
                        <router-link
                            :to="'/dashboard/ContributionPreview/' + item.id"
                            ><div
                                class="cursor-pointer rounded-full w-8 h-8 bg-secondcolor p-3 mr-2 flex justify-center items-center"
                            >
                                <FontAwesomeIcon
        ...
        <ul class="flex w-full justify-center items-center gap-4 py-6 mt-4">
            ...
            <li
                v-for="index in Math.ceil(contributions.length / 5)"
                :key="index"
...
library.add(faBookmark, faArrowRight, faAngleLeft, faAngleRight);
...
export default {
    mounted() {
        axios.get("/api/getOverview/Contribution").then((response) => {
   ...
   /* computed: {
        filteredItems() {
            return this.contributions
                .filter((contribution) => {
                    return contribution.date.includes(
                        this.searchDate.toString()
                    );
                })
                .filter((contribution) => {
                    return contribution.categorie.includes(this.categorie);
                })
                .filter((contribution) => {
                    return contribution.statut.includes(this.statut);
                });
        },
    },*/
    methods: {
        ...
        mod(p) {
            return (
                ((p % Math.ceil(this.contributions.length / 5)) +
                    Math.ceil(this.contributions.length / 5)) %
                Math.ceil(this.contributions.length / 5)
            );
        },
    },
